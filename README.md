...
## 6. EDA 
  ...
  - 정답값으로 쓰려는 Credit_Score에 결측치가 하나 존재했는데 Monthly_Balance역시 결측치가 존재해서 결측치는 제거하고 보았습니다.
...
## 7. FeatureSelection 및 모델 선정기준
  ...
  - 연속형과 범주형 컬럼들이 공존해서 tabtransformer모델을 사용 했습니다.
...
## 8. 모델링 및 학습
  - 성능 향상 및 연속형 컬럼과 범주형 컬럼이 공존해서 tabtranformer 모델을 사용했습니다.
    이 때 빠른 출력 및 과적합 방지를 위해서 depths = 3, heads = 4, dim=16으로 설정했고 또한 특정 neuron의 의존 방지를 위해 dropout을 0.1로 설정했습니다.
<img width="466" height="357" alt="탭트랜스포머" src="https://github.com/user-attachments/assets/2ccddb3e-6390-44df-bb80-c0265694eaf3" />

  - 손실함수를 계산하기 위해 nn.CrossEntropyLoss()사용했고 옵티마이저 학습률은 0.001로 설정했습니다.
    <img width="320" height="150" alt="로스 및 학습률" src="https://github.com/user-attachments/assets/114c7ce4-eef7-4921-8439-ff7315e76d77" />
  
  - 모델이 무거워 배치사이즈를 1024로 설정하고 TabTransformer를 사용하기 위해 데이터를 범주형과 연속형으로 따로 설정해줬습니다.
    <img width="318" height="645" alt="배치사이즈 및 데이터로더" src="https://github.com/user-attachments/assets/1b5b4bf7-c92e-48ac-b9bd-441257473e01" />

  - 모델의 학습시간 단축 및 과적합 방지를 위해서 에폭을 50으로 설정했습니다.

     <img width="215" height="70" alt="image" src="https://github.com/user-attachments/assets/795c61da-08fd-4e10-8424-4fa11be2ec79" />
...
## 10. 개선사항 
  ...
  - 조기에 학습을 끝내는 early stopping 방식을 과적합 방지를 위해 설정해보면 좋을 것 같습니다.  
